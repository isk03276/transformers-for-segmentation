...
class OccludedBTCVDataset(BTCVDataset):
    ...
    def get_image(
        self, image_file_name: str, aorta_index_array, inferior_vena_cava_index_array
    ):
    ...
    def __getitem__(self, index: Union[int, torch.Tensor]):
        ...
        label, aorta_index_array, inferior_vena_cava_index_array = self.get_label(
            label_file_name
        )
        image = self.get_image(
            image_file_name, aorta_index_array, inferior_vena_cava_index_array
        )
        if self.transform:
    ...
    def make_occlusion(self, image, target_index_array):
        ...
        middle_occlusion_location = int(
            len(target_slices_index) * self.occlusion_location
        )
        occlusion_half_size = int(len(target_slices_index) * self.occlusion_ratio / 2)
        occlusion_target_slices_index = np.array(
            target_slices_index[
                middle_occlusion_location
                - occlusion_half_size : middle_occlusion_location
                + occlusion_half_size
            ]
        )
